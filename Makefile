...
TEST_SOCKWRAP = $(BINDIR)/test_sockwrap

# Default target
.PHONY: all clean test dirs debug release

all: dirs $(TEST_RUNNER) $(TEST_BENCODE) $(TEST_BENCODE_EXTENDED) $(TEST_SHA1) $(TEST_UTILS) $(TEST_LOGGING) $(TEST_METAINFO) $(TEST_FILEMGR) $(TEST_PROTOCOL) $(TEST_INTEGRATION) $(TEST_SOCKET_INTEGRATION) $(TEST_PEER_INTEGRATION) $(TEST_FILE_PROTOCOL_INTEGRATION) $(TEST_ERROR_SCENARIOS) $(TEST_STRESS) $(TEST_LARGE_FILES) $(TEST_FUZZ) $(TEST_SOCKWRAP)
...
$(TEST_SOCKWRAP): $(TESTDIR)/test_sockwrap.pas $(SOCKWRAP_SRC) $(TESTDIR)/testframework.pas
	$(FPC) $(FPCFLAGS) -Fu$(SRCDIR) -Fu$(TESTDIR) -o$@ $<
